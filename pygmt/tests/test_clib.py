...
def test_create_data_fails():
    """Check that create_data raises exceptions for invalid input and
    output."""
...
def test_virtualfile_from_vectors_one_string_column():
    """Test passing in one column with string dtype into virtual file
    dataset."""
    size = 5
...
def test_virtualfile_from_vectors_two_string_columns():
    """Test passing in two columns of string dtype into virtual file
    dataset."""
    size = 5
    x = np.arange(size, dtype=np.int32)
    y = np.arange(size, size * 2, 1, dtype=np.int32)
...
def test_extract_region_two_figures():
    """Extract region should handle multiple figures existing at the same
    time."""
...
def test_dataarray_to_matrix_negative_x_and_y_increment():
    """Check that dataarray_to_matrix returns correct output with flipped
    x/y."""
    data = np.diag(v=np.arange(3))
    x = np.linspace(start=4, stop=0, num=3)
    y = np.linspace(start=9, stop=5, num=3)
...
def test_get_default():
    """Make sure get_default works without crashing and gives reasonable
    results."""
    with clib.Session() as lib:
